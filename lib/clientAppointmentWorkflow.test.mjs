...
test("client reschedule availability reuses the canonical slots engine", () => {
  assert.match(clientMove, /export async function GET/);
  assert.match(clientMove, /new URL\("\/api\/slots"/);
  assert.match(clientMove, /filterClientRescheduleSlots/);
  assert.doesNotMatch(clientMove, /snapshot\.cancellation_policy[\s\S]{0,100}requestedStart/);
});
