...
test('task disconnect immediately updates status and restores normal connection action',async()=>{
  ...
  },{fetch:async url=>{requests.push(url);return {ok:true,json:async()=>url.endsWith('/status')?{connected:true,email:'a@example.invalid'}:url.endsWith('/disconnect')?{connected:false,remoteRevocation:'complete'}:{url:'https://accounts.google.com/fixture'}};},window:{location:{assign:url=>requests.push(url)}}}).default;
